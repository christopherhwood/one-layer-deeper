"""Endpoint-trained serial reducer with local finite-state action ripples.
...
tape. Alternating tied left-to-right and right-to-left scans propagate local
information across the digit tape. At every position, the update can see only a
small shared categorical action rather than an unrestricted prompt-level
controller. A generic mutual-information regularizer encourages sharp,
balanced action use without assigning arithmetic meanings. All updates remain
learned from endpoint labels, and the same transition is reused for every T.
"""
    ...
    def forward(
        ...
        for column_index in range(self.columns - 1, -1, -1):
            ...
            for micro_step in range(MICRO_STEPS):
                ...
                scan_source = self.scan_input(
                    torch.cat(
                        (normalized, modulus_features, injected), dim=-1
                    )
                )
                if micro_step % 2:
                    scan_source = scan_source.flip(1)
                scanned, _ = self.scan(scan_source)
                if micro_step % 2:
                    scanned = scanned.flip(1)
                action_probability = F.softmax(
                    self.action_head(scanned) / temperature, dim=-1
                )
                actions.append(action_probability)
                control = action_probability @ self.action_embedding
