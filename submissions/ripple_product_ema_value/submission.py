"""Product-aware ripple squaring with EMA and endpoint value supervision.

The student and temporal teacher are unchanged from the EMA candidate.  The only
additional signal maps learned endpoint digit probabilities and evaluator-supplied
endpoint labels to bounded soft decimal values.  It introduces no arithmetic
rollout, generated answer, or process label.
"""
...
class Model(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            with torch.no_grad():
                teacher_register, _ = self._run_square(
                    self.teacher_square,
                    initial_register,
                    modulus,
                    t_values,
...
def _sequence_value_loss(batch: TokenLossBatch) -> Tensor:
    """Dense numeric distance using only supplied endpoint digit labels."""

    digit_valid = (
        batch.valid_mask
        & (batch.labels >= DIGIT_OFFSET)
        & (batch.labels < DIGIT_OFFSET + NUM_DIGITS)
    )
    digit_rank = digit_valid.long().cumsum(dim=1) - 1
    place_weight = torch.pow(
        torch.full_like(batch.logits[..., 0], 0.1, dtype=torch.float32),
        (digit_rank + 1).clamp_min(1),
    )
    place_weight = place_weight * digit_valid.to(place_weight.dtype)
    digit_probabilities = F.softmax(
        batch.logits[..., DIGIT_OFFSET : DIGIT_OFFSET + NUM_DIGITS], dim=-1
    )
    digit_values = torch.arange(
        NUM_DIGITS, device=batch.logits.device, dtype=digit_probabilities.dtype
    )
    expected_digits = (digit_probabilities * digit_values).sum(dim=-1)
    true_digits = (batch.labels - DIGIT_OFFSET).clamp(0, NUM_DIGITS - 1).float()
    predicted_value = (expected_digits * place_weight).sum(dim=1)
    true_value = (true_digits * place_weight).sum(dim=1)
    rows = digit_valid.any(dim=1)
    row_losses = F.smooth_l1_loss(
        predicted_value, true_value, reduction="none", beta=0.1
    )
    weights = rows.to(row_losses.dtype)
    return (row_losses * weights).sum() / weights.sum().clamp_min(1.0)
...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    ...
    return loss + EMA_WEIGHT * scale * consistency + VALUE_WEIGHT * value
