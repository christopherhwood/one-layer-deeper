...
class ProductRippleSquare(nn.Module):
    ...
    def __init__(self, width: int) -> None:
        ...
        self.ssm_norm = RMSNorm(HIDDEN)
        self.ssm_in = nn.Linear(HIDDEN, 4 * HIDDEN, bias=False)
        self.ssm_out = nn.Linear(2 * HIDDEN, HIDDEN, bias=False)
        self.decay_bias = nn.Parameter(torch.full((HIDDEN,), 2.0))
    ...
    def _selective_scan(self, parameters: Tensor) -> Tensor:
        value, decay, write_gate, read_gate = parameters.chunk(4, dim=-1)
        memory = torch.zeros_like(value[:, 0])
        outputs: list[Tensor] = []
        for position in range(self.columns):
            retention = torch.sigmoid(decay[:, position] + self.decay_bias)
            write = torch.tanh(value[:, position]) * torch.sigmoid(
                write_gate[:, position]
            )
            memory = retention * memory + (1.0 - retention) * write
            outputs.append(torch.sigmoid(read_gate[:, position]) * memory)
    ...
    def forward(
        ...
        for _ in range(refinement_steps):
            parameters = self.ssm_in(self.ssm_norm(state))
            forward = self._selective_scan(parameters)
            backward = self._selective_scan(parameters.flip(1)).flip(1)
            state = state + torch.sigmoid(self.scan_gate) * self.ssm_out(
