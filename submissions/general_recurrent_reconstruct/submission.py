"""Generic recurrent transition with same-prompt representation learning.

The learned transition sees a current digit register and immutable modulus
context, but no arithmetic-specific features or process labels.  A shared
attention block repeatedly refines answer and scratch slots.  Every refinement
of the requested final transition predicts the observed endpoint, and recurrent
state is detached between refinements to keep each learning path shallow. The
scratch workspace also reconstructs the untouched supplied N/x digit registers
during the first transition. This generic same-prompt objective is scheduled
down after early representation learning and adds no generated or arithmetic
labels. The same complete transition is then reused exactly T times; T is only
a compute-control signal and is never exposed to the learned transition.
"""
...
class DeepRecurrentTransition(nn.Module):
    ...
    def __init__(self, digits: int) -> None:
        ...
        self.reconstruction_norm = RMSNorm(WIDTH)
        self.reconstruct_modulus = nn.Linear(WIDTH, NUM_DIGITS)
        self.reconstruct_source = nn.Linear(WIDTH, NUM_DIGITS)
    ...
    def forward(
        ...
        reconstruction_logits: list[tuple[Tensor, Tensor]] = []
        for segment in range(SEGMENTS):
            ...
            if reconstruct_prompt:
                reconstruction_state = self.reconstruction_norm(scratch)
                reconstruction_logits.append(
                    (
                        self.reconstruct_modulus(reconstruction_state),
                        self.reconstruct_source(reconstruction_state),
                    )
                )
        ...
        return probabilities, tuple(phase_logits), tuple(reconstruction_logits)
...
class Model(nn.Module):
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.reconstruction_weight = 0.05
    ...
    def forward(
        ...
        prompt_reconstruction: tuple[tuple[Tensor, Tensor], ...] = ()
        for outer_step in range(outer_steps):
            source = register.detach() if self.training and outer_step > 0 else register
            candidate, phases, reconstruction = self.transition(
                source,
                modulus,
                temperature,
                self.training and self.detach_segments,
                self.training and outer_step == 0,
            )
            if outer_step == 0:
        ...
        return logits, {
            ...
            "reconstruction_logits": prompt_reconstruction,
            "modulus_targets": modulus.argmax(dim=-1),
            "source_targets": supplied_source.argmax(dim=-1),
...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    ...
    reconstruction_terms: list[Tensor] = []
    modulus_targets = batch.auxiliary["modulus_targets"]
    source_targets = batch.auxiliary["source_targets"]
    for modulus_logits, source_logits in batch.auxiliary["reconstruction_logits"]:
        reconstruction_terms.extend(
            (
                F.cross_entropy(
                    modulus_logits.transpose(1, 2), modulus_targets
                ),
                F.cross_entropy(
                    source_logits.transpose(1, 2), source_targets
                ),
            )
        )
    representation = torch.stack(reconstruction_terms).mean()
    weight = batch.auxiliary["reconstruction_weight"]
    return endpoint + 0.5 * torch.stack(deep_terms).mean() + weight * representation
...
class WallClockSchedule:
    ...
    def step(self) -> None:
        ...
        if fraction < 0.05:
            self.model.reconstruction_weight = 0.05 + 5.0 * fraction
        elif fraction < 0.60:
            progress = (fraction - 0.05) / 0.55
            self.model.reconstruction_weight = 0.30 - 0.25 * progress
        else:
            self.model.reconstruction_weight = 0.05
