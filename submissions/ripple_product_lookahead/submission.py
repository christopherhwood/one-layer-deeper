...
class LookaheadAdamW:
    """AdamW fast weights with periodic interpolation toward slow weights."""
        ...
        for group in self.param_groups:
            for parameter in group["params"]:
                ...
                if "lookahead_slow" not in state:
                    state["lookahead_slow"] = parameter.detach().clone()
        if self.updates % self.sync_period != 0:
            return
        for group in self.param_groups:
            for parameter in group["params"]:
                slow = self.inner.state[parameter]["lookahead_slow"]
                slow.lerp_(parameter, self.slow_rate)
...
def build_optimizer(model: Model, spec: OptimizerSpec) -> OptimizerBundle:
    ...
    optimizer = LookaheadAdamW(inner)
