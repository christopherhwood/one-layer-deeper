"""Product-aware ripple squaring with compute-sparse EMA consistency.
...
The teacher forward and consistency term run on alternating optimizer updates;
inactive updates preserve the complete student objective while skipping teacher
compute. No arithmetic process state or intermediate label is supplied.
"""
...
class Model(nn.Module):
    ...
    def forward(
        ...
        if self.training and self.teacher_active:
            with torch.no_grad():
        ...
        auxiliary["teacher_active"] = self.training and self.teacher_active
...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    ...
    if batch.auxiliary["teacher_active"]:
        ...
        loss = loss + EMA_WEIGHT * scale * consistency
    return loss
...
class WallClockSchedule:
    ...
    def step(self) -> None:
        ...
        # The scheduler runs after each completed optimizer update. With the
        # teacher initially active, this selects updates 1, 3, 5, ... .
        self.model.teacher_active = self.updates % 2 == 0
