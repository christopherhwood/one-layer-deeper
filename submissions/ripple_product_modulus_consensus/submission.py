...
The optimizer symmetrically projects conflicting family gradients before their
mean AdamW update, targeting cross-modulus function directions rather than
prompt-specific memorization. No arithmetic process state or intermediate label
is supplied.
"""
...
class ConsensusAdamW(torch.optim.AdamW):
    """Symmetrically remove conflicts between disjoint modulus families."""
    ...
    def step(self, closure=None):
        ...
        for parameter, first in zip(parameters, self.first_gradients):
            ...
            conflict = dot.clamp_max(0.0)
            first_norm = first.square().sum().clamp_min(1.0e-12)
            second_norm = second.square().sum().clamp_min(1.0e-12)
            # Each family keeps its orthogonal component. Only the component
            # that would increase the other family's loss is removed.
            projected_first = first - conflict / second_norm * second
            projected_second = second - conflict / first_norm * first
