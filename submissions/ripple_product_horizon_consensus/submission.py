"""Product-aware ripple/EMA squaring with horizon-separated gradients.
...
Two evaluator-owned passes use disjoint horizon groups from the same untouched
minibatch. The first differentiates genuine T=1 endpoints and refinement heads;
the second differentiates genuine T=2/3 endpoints. The custom optimizer keeps
the one-square gradient intact and projects only a conflicting composed-horizon
component before AdamW. No arithmetic process state or intermediate label is
supplied.
"""
...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    ...
    horizon_pass = int(batch.auxiliary["loss_half"])
    row_mask = (t_values == 1) if horizon_pass == 0 else (t_values > 1)
...
class HorizonPCGradAdamW(torch.optim.AdamW):
    """Protect direct one-step learning from conflicting composed horizons."""

    def __init__(self, groups, model: Model, **kwargs) -> None:
        super().__init__(groups, **kwargs)
        self.model = model
        self.shallow_gradients: list[Tensor | None] = []
    ...
    def between_backward_passes(self, context: BackwardPassContext) -> None:
        if context.pass_index != 1 or context.total_passes != 2:
            raise RuntimeError("horizon routing requires exactly two passes")
        if self.model.loss_half != 0:
            raise RuntimeError("first horizon pass must use T=1 rows")
        self.shallow_gradients = [
    ...
    def step(self, closure=None):
        ...
        if len(self.shallow_gradients) != len(parameters):
            self.model.loss_half = 0
            raise RuntimeError("T=1 gradients were not captured")

        gradient_dot = torch.zeros((), device=parameters[0].device)
        shallow_norm_square = torch.zeros_like(gradient_dot)
        for parameter, shallow in zip(parameters, self.shallow_gradients):
            deep = parameter.grad
            if shallow is None:
                continue
            shallow_norm_square.add_(shallow.float().square().sum())
            if deep is not None:
                gradient_dot.add_((shallow.float() * deep.float()).sum())

        projection = (
            gradient_dot.clamp_max(0.0)
            / shallow_norm_square.clamp_min(1.0e-12)
        )
        for parameter, shallow in zip(parameters, self.shallow_gradients):
            deep = parameter.grad
            if shallow is None:
                if deep is not None:
                    deep.mul_(DEEP_HORIZON_SCALE)
                continue
            combined = shallow.clone()
            if deep is not None:
                projected_deep = deep - projection.to(deep.dtype) * shallow
                combined.add_(projected_deep, alpha=DEEP_HORIZON_SCALE)
