"""Product-aware ripple/EMA squaring with learned numeric token geometry.
...
Each digit interface additionally receives scalar and low-frequency Fourier
coordinates through learned projections. The arithmetic topology, exact losses,
and EMA objective are otherwise unchanged. No arithmetic process state or
intermediate label is supplied.
"""
...
class ProductRippleSquare(nn.Module):
    ...
    def __init__(self, width: int) -> None:
        ...
        self.modulus_numeric_projection = nn.Linear(
            NUMERIC_FEATURES, HIDDEN, bias=False
        )
        self.register_numeric_projection = nn.Linear(
            NUMERIC_FEATURES, HIDDEN, bias=False
        )
        self.pair_numeric_projection = nn.Linear(
            2 * NUMERIC_FEATURES, HIDDEN, bias=False
        )
    ...
    @staticmethod
    def _numeric_table(reference: Tensor) -> Tensor:
        values = torch.arange(
            NUM_DIGITS,
    ...
    def _numeric_features(self, digits: Tensor) -> Tensor:
        return digits @ self._numeric_table(digits)

    def _product_columns(self, register: Tensor) -> Tensor:
        batch = register.shape[0]
        numeric = self._numeric_table(register)
        left = numeric[:, None, :].expand(-1, NUM_DIGITS, -1)
        right = numeric[None, :, :].expand(NUM_DIGITS, -1, -1)
        numeric_pairs = self.pair_numeric_projection(
        ...
        pair_features = torch.einsum(
            ...
            register,
            register,
            self.pair_embedding + numeric_pairs,
        )
        significance = (
        ...
        state = self.input_projection(
            torch.cat(
                (
                    ...
                    + self.modulus_numeric_projection(
                        self._numeric_features(modulus_columns)
                    ),
                    self.register_projection(register_columns)
                    + self.register_numeric_projection(
                        self._numeric_features(register_columns)
                    ),
                ),
                dim=-1,
            )
