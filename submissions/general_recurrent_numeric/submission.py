"""Generic recurrent transition with a learned ordered-number interface.
...
state is detached between refinements to keep each learning path shallow.
Digit tokens receive generic scalar and low-frequency Fourier coordinates,
projected by learned weights, and endpoint distributions receive an auxiliary
ordinal loss. No task arithmetic or intermediate label is encoded. The same
complete transition is then reused exactly T times; T is only a compute-control
...
class DeepRecurrentTransition(nn.Module):
    ...
    def __init__(self, digits: int) -> None:
        ...
        self.numeric_projection = nn.Linear(NUMERIC_FEATURES, WIDTH, bias=False)
    ...
    @staticmethod
    def _numeric_features(probabilities: Tensor) -> Tensor:
        """Expected generic coordinates for a hard or soft decimal token."""
        ...
        table = torch.stack(
            (
                ...
                torch.sin(2.0 * math.pi * values),
                torch.cos(2.0 * math.pi * values),
            ),
            dim=-1,
        )
        return probabilities @ table

    def _embed_digits(self, probabilities: Tensor) -> Tensor:
        return self.digit_projection(probabilities) + self.numeric_projection(
            self._numeric_features(probabilities)
        )
    ...
    def forward(
        ...
        modulus_context = (
            self._embed_digits(modulus)
            + position
            + self.role_embedding[0]
        )
        source_context = (
            self._embed_digits(source)
            + position
        ...
        for segment in range(SEGMENTS):
            ...
            snapped = (
                self._embed_digits(probabilities)
                + position
                + self.role_embedding[2]
            )
...
def _ordinal_endpoint_loss(logits: Tensor, labels: Tensor, valid: Tensor) -> Tensor:
    """One-dimensional Wasserstein distance on the observed digit targets."""

    digit_logits = logits[..., DIGIT_OFFSET : DIGIT_OFFSET + NUM_DIGITS]
    probabilities = F.softmax(digit_logits, dim=-1)
    target_digits = (labels - DIGIT_OFFSET).clamp(0, NUM_DIGITS - 1)
    targets = F.one_hot(target_digits, NUM_DIGITS).to(probabilities.dtype)
    distance = (
        probabilities.cumsum(dim=-1) - targets.cumsum(dim=-1)
    ).abs().mean(dim=-1)
    weights = valid.to(distance.dtype)
...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    ...
    aligned_phases = [_target_aligned(phase, batch) for phase in phases]
    deep_terms = [
        _masked_cross_entropy(
            phase, batch.labels, batch.valid_mask
        )
        for phase in aligned_phases
    ]
    ordinal_terms = [
        _ordinal_endpoint_loss(
            phase, batch.labels, batch.valid_mask
        )
        for phase in (batch.logits, *aligned_phases)
    ]
    return (
        endpoint
        + 0.5 * torch.stack(deep_terms).mean()
        + ORDINAL_WEIGHT * torch.stack(ordinal_terms).mean()
    )
