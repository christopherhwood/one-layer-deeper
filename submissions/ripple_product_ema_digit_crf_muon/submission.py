"""Product-aware ripple/EMA squaring with a hybrid Muon/AdamW optimizer.
...
The recurrent register remains unchanged and factorized. The surviving
first-order observed-digit CRF scores only the final fixed-width output. The
only experiment is optimizer geometry: pinned torch.optim.Muon updates named
hidden two-dimensional transforms, while the numeric interface, output heads,
normalizers, biases, and gates retain the tracked AdamW dynamics. No arithmetic
...
CRF_WEIGHT = 0.2
MUON_PARAMETER_NAMES = frozenset(
    {
        "square.input_projection.weight",
        "square.scan.weight_ih_l0",
        "square.scan.weight_hh_l0",
        "square.scan.weight_ih_l0_reverse",
        "square.scan.weight_hh_l0_reverse",
...
class Model(nn.Module):
    ...
    def forward(
        ...
        crf = self.square.crf_energies(terminal_state, terminal_unary)
...
def _partial_suffix_crf_nll(
    ...
    if row_mask is None:
        row_mask = torch.ones(
            unary.shape[0], device=unary.device, dtype=torch.bool
        )
    valid = batch.valid_mask & row_mask[:, None]
...
class CompositeOptimizer:
    """One evaluator-visible optimizer backed by disjoint Muon and AdamW."""
    ...
    def state_dict(self) -> dict[str, object]:
        return {
            "muon": self.optimizers[0].state_dict(),
...
def build_optimizer(model: Model, spec: OptimizerSpec) -> OptimizerBundle:
    ...
    missing = MUON_PARAMETER_NAMES - named_trainable.keys()
    if missing:
        raise RuntimeError(f"missing named Muon parameters: {sorted(missing)}")
    ...
    muon = torch.optim.Muon(
        [{"params": muon_parameters, "base_lr": BASE_LR}],
        lr=BASE_LR * 0.1,
        weight_decay=0.1,
        momentum=0.95,
        nesterov=True,
    ...
    optimizer = CompositeOptimizer(muon, adamw)
