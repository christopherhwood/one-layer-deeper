"""Product-aware ripple squaring with endpoint and phase EMA consistency.

The full four-round EMA teacher and endpoint KL are preserved.  On T=1 rows,
corresponding first-square student and teacher refinement phases are additionally
aligned.  Every such phase already predicts the supplied square endpoint, so the
signal introduces no process label or arithmetic state target.
"""
...
class Model(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            with torch.no_grad():
                teacher_register, teacher_first_phases = self._run_square(
                    self.teacher_square,
                    initial_register,
                    modulus,
                    t_values,
            ...
            auxiliary["teacher_phase_logits"] = tuple(
                self._place_logits(phase, lengths, prompt_length)
                for phase in teacher_first_phases
            )
...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    ...
    teacher_phase_terms = [
        _teacher_consistency(
            _target_aligned(student_phase, batch),
            _target_aligned(teacher_phase, batch),
            t1_mask,
        )
        for student_phase, teacher_phase in zip(
            batch.auxiliary["phase_logits"],
            batch.auxiliary["teacher_phase_logits"],
        )
    ]
    phase_consistency = (
        torch.stack(teacher_phase_terms).mean()
        if teacher_phase_terms
        else consistency.new_zeros(())
    )
    scale = float(batch.auxiliary["consistency_scale"])
    return (
        loss
        + EMA_WEIGHT * scale * consistency
        + PHASE_EMA_WEIGHT * scale * phase_consistency
    )
