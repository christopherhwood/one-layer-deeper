"""Position-only recurrent mixer with fixed-random local feedback.
...
endpoint digit through a fixed random classifier. This supplies a stable local
error coordinate without an intermediate arithmetic label or custom backward.
"""
    ...
    def forward(
        ...
        return self._place(register.clamp_min(1e-8).log(), lengths, prompt), {
            ...
            "local_phase_logits": tuple(
                self._place(phase, lengths, prompt)
...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    ...
    local_phases = [
        _masked_ce(_target_aligned(item, batch), batch.labels, t1)
        for item in batch.auxiliary["local_phase_logits"]
    ]
    if local_phases:
        loss = loss + LOCAL_FEEDBACK_WEIGHT * torch.stack(local_phases).mean()
    return loss
