"""Product-aware ripple squaring with an EMA-teacher burn-in.

The full EMA baseline is preserved, but its consistency loss remains disabled
through the first 15 percent of wall time and ramps to full strength from 15 to
40 percent.  EMA parameters still update from the first optimizer step, avoiding
student pressure toward the teacher's initially random predictions.
"""
...
CONSISTENCY_BURN_IN = 0.15
CONSISTENCY_FULL_AT = 0.40
...
class WallClockSchedule:
    ...
    def step(self) -> None:
        ...
        if fraction <= CONSISTENCY_BURN_IN:
            self.model.consistency_scale = 0.0
        else:
            ramp = (fraction - CONSISTENCY_BURN_IN) / (
                CONSISTENCY_FULL_AT - CONSISTENCY_BURN_IN
            )
            self.model.consistency_scale = min(ramp, 1.0)
