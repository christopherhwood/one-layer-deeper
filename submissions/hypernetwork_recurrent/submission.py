"""Modulus-conditioned recurrent reasoner for repeated composition.

The learned transition combines a small bidirectional context encoder with a
Neural-GPU-style cellular state machine.  The context encoder produces FiLM
parameters that condition every recurrent update, forcing the computation to
depend on the supplied context rather than allowing it to be ignored as an
ordinary token.  Nothing encodes multiplication, carries, comparison, or
modular reduction.  The same conditioned transition is reused for every outer
application and receives endpoint supervision after each recurrent sweep.
"""
...
class CellularUpdate(nn.Module):
    ...
    def forward(
        self, state: Tensor, context: Tensor, condition: Tensor
    ) -> Tensor:
        ...
        gates = (
            local * (1.0 + 0.5 * torch.tanh(scale[:, None]))
            + shift[:, None]
            + self.context(context)
            + self.global_mix(global_state)
        )
...
class CellularTransition(nn.Module):
    ...
    def __init__(self, digits: int) -> None:
        ...
        self.modulus_encoder = nn.GRU(
            HIDDEN,
            HIDDEN // 2,
            batch_first=True,
            bidirectional=True,
        )
        self.condition_norm = RMSNorm(HIDDEN)
    ...
    def encode_modulus(self, modulus: Tensor) -> tuple[Tensor, Tensor]:
        boundary = self._boundaries(modulus).expand(modulus.shape[0], -1, -1)
        encoded, _ = self.modulus_encoder(
            self.modulus_projection(modulus) + boundary
        )
        condition = self.condition_norm(encoded.mean(dim=1))
        return encoded, condition

    def forward(
        self,
        source: Tensor,
        modulus_features: Tensor,
        condition: Tensor,
        temperature: float,
        ...
        for _ in range(SWEEPS):
            for _ in range(self.digits):
                state = self.cell(state, context, condition)
...
class Model(nn.Module):
    ...
    def forward(
        ...
        modulus_features, condition = self.transition.encode_modulus(modulus)
        outer_steps = (
        ...
        for outer_step in range(outer_steps):
            candidate, phases = self.transition(
                register, modulus_features, condition, temperature
            )
