"""Discrete-action serial reducer with a globally normalized digit chain.
...
from endpoint labels, and the same transition is reused for every T. Adjacent
final digits are scored jointly by a learned first-order CRF and decoded with
Viterbi, testing whether broad digit improvements form coherent sequences.
"""
...
class ProductRippleSquare(nn.Module):
    ...
    def forward(
        ...
        output_state = self.edge_norm(state).flip(1)
        adjacent = torch.cat(
...
class Model(nn.Module):
    ...
    def forward(
        ...
        for outer_step in range(loops):
            ...
            candidate, phases, actions, crf = self.square(
            ...
            if outer_step == 0:
                ...
                terminal_crf = (
                    torch.zeros_like(crf[0]),
                    torch.zeros_like(crf[1]),
                    crf[2],
                    crf[3],
                )
        ...
        if self.training:
            ...
            path = self._viterbi(*terminal_crf)
...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    ...
    return (
        loss
        + ACTION_WEIGHT * action_information
        + CRF_WEIGHT * crf
    )
