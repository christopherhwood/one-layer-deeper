"""Product-aware ripple/EMA squaring with hybrid Shampoo/AdamW updates.
...
The experiment changes only optimizer geometry: the five dense reducer
transforms use Kronecker-factored Shampoo updates with AdamW norm grafting,
while embeddings, heads, gates, and normalizers retain AdamW. No arithmetic
process state or intermediate label is supplied.
"""
...
CRF_WEIGHT = 0.2
SHAMPOO_LR = 3.0e-3
SHAMPOO_PARAMETER_NAMES = frozenset(
    {
        "square.input_projection.weight",
        "square.qkv.weight",
        "square.attention_out.weight",
        "square.mlp.0.weight",
        "square.mlp.2.weight",
    }
)
...
class Model(nn.Module):
    ...
    def forward(
        ...
        refinement_steps = (
            int(torch.randint(2, 7, (), device=input_ids.device).item())
            if self.training
...
class Shampoo(torch.optim.Optimizer):
    """Small-matrix Kronecker Shampoo with first-order norm grafting.

    Each matrix keeps row and column gradient covariances.  Their inverse
    fourth roots whiten the two axes of the gradient; matching the resulting
    norm to the raw gradient keeps the step scale comparable to AdamW while
    testing the genuinely different second-order direction.
    """
    ...
    @staticmethod
    def _inverse_fourth_root(matrix: Tensor, eps: float) -> Tensor:
        dimension = matrix.shape[0]
        regularized = matrix + eps * torch.eye(
            dimension, device=matrix.device, dtype=matrix.dtype
        )
        eigenvalues, eigenvectors = torch.linalg.eigh(regularized)
        scale = eigenvalues.clamp_min(eps).pow(-0.25)
        return (eigenvectors * scale.unsqueeze(0)) @ eigenvectors.mT
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            eps = float(group["eps"])
            frequency = int(group["root_frequency"])
            lr = float(group["lr"])
            ...
            for parameter in group["params"]:
                ...
                if gradient is None:
                    continue
                if gradient.ndim != 2:
                    raise RuntimeError("Shampoo parameters must be matrices")
                ...
                if not state:
                    ...
                    state["left_root"] = torch.eye(
                        rows, device=gradient.device, dtype=torch.float32
                    )
                    state["right_root"] = torch.eye(
                        columns, device=gradient.device, dtype=torch.float32
                    )
                    state["momentum"] = torch.zeros_like(gradient)

                state["step"] += 1
                state["left"].mul_(beta).addmm_(
                    gradient, gradient.mT, beta=1.0, alpha=(1.0 - beta) / columns
                )
                state["right"].mul_(beta).addmm_(
                    gradient.mT, gradient, beta=1.0, alpha=(1.0 - beta) / rows
                )
                if state["step"] == 1 or state["step"] % frequency == 0:
                    state["left_root"] = self._inverse_fourth_root(
                        state["left"], eps
                    )
                    state["right_root"] = self._inverse_fourth_root(
                        state["right"], eps
                    )

                direction = state["left_root"] @ gradient @ state["right_root"]
                raw_norm = torch.linalg.vector_norm(gradient)
                direction_norm = torch.linalg.vector_norm(direction)
                direction.mul_(raw_norm / direction_norm.clamp_min(1e-12))
                state["momentum"].mul_(momentum).add_(
                    direction, alpha=1.0 - momentum
                )
                if weight_decay:
                    parameter.mul_(1.0 - lr * weight_decay)
                parameter.add_(
                    state["momentum"].to(dtype=parameter.dtype), alpha=-lr
                )
        return loss


class CompositeOptimizer:
    """One evaluator-visible optimizer backed by disjoint optimizers."""

    def __init__(
        self,
        shampoo: torch.optim.Optimizer,
        adamw: torch.optim.Optimizer,
    ) -> None:
        self.optimizers = [shampoo, adamw]
        self.param_groups = [*shampoo.param_groups, *adamw.param_groups]
    ...
    def state_dict(self) -> dict[str, object]:
        return {
            "shampoo": self.optimizers[0].state_dict(),
            "adamw": self.optimizers[1].state_dict(),
        }
...
def build_optimizer(model: Model, spec: OptimizerSpec) -> OptimizerBundle:
    ...
    missing = SHAMPOO_PARAMETER_NAMES - named_trainable.keys()
    if missing:
        raise RuntimeError(f"missing named Shampoo parameters: {sorted(missing)}")
    shampoo_parameters: list[Tensor] = []
    decay: list[Tensor] = []
    no_decay: list[Tensor] = []
    for name, parameter in named_trainable.items():
        if name in SHAMPOO_PARAMETER_NAMES:
            if parameter.ndim != 2:
                raise RuntimeError(f"Shampoo parameter is not 2D: {name}")
            shampoo_parameters.append(parameter)
        else:
            (decay if parameter.ndim >= 2 else no_decay).append(parameter)

    shampoo = Shampoo(
        [{"params": shampoo_parameters, "base_lr": SHAMPOO_LR}],
        lr=SHAMPOO_LR * 0.1,
    )
    ...
    optimizer = CompositeOptimizer(shampoo, adamw)
