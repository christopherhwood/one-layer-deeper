"""Position-only recurrent mixer with fitted attainable-state training.
...
refinement depth, and outer T. Late in training, the reducer is initialized
from a small mixture of the current input state and Gaussian states fitted to
the reducer's own clean final-state distribution. Evaluation remains clean.
"""
...
STATE_EMA_RATE = 0.05
MAX_STATE_MIX = 0.35
CLEAN_ROW_PROBABILITY = 0.10
RECOVERY_LOSS_WEIGHT = 0.15
...
class ProductPositionSquare(nn.Module):
    def __init__(self, width: int) -> None:
        ...
        self.register_buffer(
            "attainable_scratch_mean", torch.zeros(self.columns, HIDDEN)
        )
        self.register_buffer(
            "attainable_scratch_variance", torch.ones(self.columns, HIDDEN)
        )
        self.state_mix = 0.0

    def _fitted_initial_state(self, condition: Tensor) -> Tensor:
        """Perturb reasoning scratch space without corrupting the problem."""

        if not self.training or self.state_mix <= 0.0:
            return condition
        standard_deviation = self.attainable_scratch_variance.clamp_min(
            1.0e-4
        ).sqrt()
        sampled_scratch = (
            self.attainable_scratch_mean
            + torch.randn_like(condition) * standard_deviation
        )
        perturb = (
            torch.rand(condition.shape[0], 1, 1, device=condition.device)
            >= CLEAN_ROW_PROBABILITY
        ).to(condition.dtype)
        mix = perturb * self.state_mix
        return condition + mix * sampled_scratch

    def _observe_attainable_state(
        self, state: Tensor, condition: Tensor
    ) -> None:
        """Fit position-wise moments of clean mutable scratch states."""

        if not self.training or self.state_mix > 0.0:
            return
        with torch.no_grad():
            observed = (state - condition).detach()
            mean = observed.mean(dim=0)
            variance = observed.var(dim=0, unbiased=False)
            self.attainable_scratch_mean.lerp_(mean, STATE_EMA_RATE)
            self.attainable_scratch_variance.lerp_(variance, STATE_EMA_RATE)
    ...
    def forward(
        ...
        condition = self.input_projection(
            torch.cat(
                (
                    product,
        ...
        clean_state = condition
        paired_recovery = self.training and self.state_mix > 0.0
        if paired_recovery:
            noisy_state = self._fitted_initial_state(condition)
            state = torch.cat((clean_state, noisy_state), dim=0)
        else:
            state = clean_state
        phases: list[Tensor] = []
        recovery_losses: list[Tensor] = []
        for _ in range(REFINEMENT_STEPS):
            ...
            if paired_recovery:
                clean_state, noisy_state = state.chunk(2, dim=0)
                clean_normalized = F.rms_norm(clean_state, (HIDDEN,)).detach()
                noisy_normalized = F.rms_norm(noisy_state, (HIDDEN,))
                recovery_losses.append(
                    F.mse_loss(noisy_normalized, clean_normalized)
                )
            else:
                clean_state = state
            phases.append(
                self.output(self.output_norm(clean_state[:, : self.width]))
            )
        self._observe_attainable_state(clean_state, condition)
        recovery_loss = (
            torch.stack(recovery_losses).mean()
            if recovery_losses
            else condition.new_zeros(())
        )
        return (
...
class Model(nn.Module):
    ...
    def forward(
        ...
        for outer_step in range(loops):
            ...
            candidate, phases, recovery_loss = self.square(
        ...
        return self._place(register.clamp_min(1e-8).log(), lengths, prompt), {
            ...
            "recovery_loss": torch.stack(recovery_losses).mean(),
        }
...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    ...
    loss = loss + RECOVERY_LOSS_WEIGHT * batch.auxiliary["recovery_loss"]
    return loss
...
class WallClockSchedule:
    ...
    def step(self) -> None:
        ...
        if fraction < 0.70:
            self.model.square.state_mix = 0.0
        else:
            recovery_progress = min((fraction - 0.70) / 0.15, 1.0)
            self.model.square.state_mix = MAX_STATE_MIX * recovery_progress
