"""Product-aware ripple squaring with exact activation-gradient replay.
...
The first evaluator-owned pass differentiates the genuine endpoint objective
and records the exact gradients of every T=1 refinement state.  The second
evaluator-owned pass recomputes identical values with refinement boundaries
detached, then replays those directions through local linear objectives.  This
shortens recurrent credit assignment without inventing arithmetic labels or
calling the derivative engine from submission code.
"""
...
class ProductRippleSquare(nn.Module):
    ...
    def forward(
        ...
        refinement_states = [state]
        for _ in range(REFINEMENT_STEPS):
            if detach_refinements:
                state = state.detach()
            ...
            refinement_states.append(state)
        ...
        return probabilities, tuple(phase_logits), tuple(refinement_states), (
...
class Model(nn.Module):
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.normalize_replay = True
        self._gradient_states: tuple[Tensor, ...] = ()
        self._saved_state_gradients: tuple[Tensor, ...] = ()
    ...
    def _run_square(
        ...
        for outer_step in range(loops):
            ...
            candidate, phases, states, crf = square(
                transition_input,
                modulus,
                temperature,
                detach_refinements=(
                    self.training
                    and square is self.square
                    and self.loss_mode == "replay"
                ),
            )
            if outer_step == 0:
                first_phases = phases
        ...
        return register, first_phases, first_states, terminal_crf
    ...
    def forward(
        ...
        if self.training and self.loss_mode == "replay":
            # The replay objective uses only first-square states on T=1 rows.
            # Later outer squares and the teacher cannot contribute gradients.
            loops = 1
        else:
            loops = (
        ...
        register, first_square_phases, first_square_states, crf = self._run_square(
            self.square,
            initial_register,
            modulus,
            t_values,
        ...
        if self.training:
            if self.loss_mode == "global":
                for state in first_square_states:
                    state.retain_grad()
                self._gradient_states = first_square_states
            else:
                auxiliary["replay_states"] = first_square_states
                auxiliary["saved_state_gradients"] = self._saved_state_gradients
                auxiliary["normalize_replay"] = self.normalize_replay
            if self.loss_mode == "global":
                with torch.no_grad():
...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    if batch.auxiliary["loss_mode"] == "replay":
        states = batch.auxiliary["replay_states"]
        gradients = batch.auxiliary["saved_state_gradients"]
        if len(states) != REFINEMENT_STEPS + 1 or len(states) != len(gradients):
            raise RuntimeError("activation replay requires every refinement state")

        t1 = (batch.auxiliary["t_values"] == 1)[:, None, None]
        masked = [gradient.detach() * t1 for gradient in gradients]
        if bool(batch.auxiliary["normalize_replay"]):
            count = t1.sum().to(torch.float32)
            count = count * states[0].shape[1] * states[0].shape[2]
            rms = [
                (gradient.float().square().sum() / count.clamp_min(1.0))
                .sqrt()
                .clamp_min(1e-12)
                for gradient in masked
            ]
            reference = rms[-1]
            directions = [
                gradient * (reference / phase_rms).to(gradient.dtype)
                for gradient, phase_rms in zip(masked, rms)
            ]
        else:
            directions = masked
        return torch.stack(
            [
                (state.float() * direction.float()).sum()
...
class GradientReplayAdamW(torch.optim.AdamW):
    """Combine the true endpoint gradient with exact local replay gradients."""
    ...
    def between_backward_passes(self, context: BackwardPassContext) -> None:
        if context.pass_index != 1 or context.total_passes != 2:
            raise RuntimeError("gradient replay requires two evaluator-owned passes")
        if self.model.loss_mode != "global":
            raise RuntimeError("first replay pass must use the global endpoint loss")
        if len(self.model._gradient_states) != REFINEMENT_STEPS + 1:
            raise RuntimeError("global pass did not expose every refinement state")

        state_gradients: list[Tensor] = []
        for state in self.model._gradient_states:
            if state.grad is None or not torch.isfinite(state.grad).all():
                raise RuntimeError("global activation gradient is missing or non-finite")
            state_gradients.append(state.grad.detach().clone())
        self.model._saved_state_gradients = tuple(state_gradients)
        self.global_gradients = [
            None if parameter.grad is None else parameter.grad.detach().clone()
            for parameter in self._parameters()
        ]
        self.model.loss_mode = "replay"
    ...
    def step(self, closure=None):
        ...
        for parameter, global_gradient in zip(parameters, self.global_gradients):
            replay_gradient = parameter.grad
            if global_gradient is None:
                if replay_gradient is not None:
                    replay_gradient.mul_(REPLAY_WEIGHT)
                continue
            combined = global_gradient.clone()
            if replay_gradient is not None:
                combined.add_(replay_gradient, alpha=REPLAY_WEIGHT)
    ...
    def _clear_replay_state(self) -> None:
        ...
        self.model._gradient_states = ()
        self.model._saved_state_gradients = ()
