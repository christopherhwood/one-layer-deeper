"""Product-aware ripple squaring with temporal EMA consistency.
...
exactly one square on only the T=1 rows in the same forward pass. The ordinary
student endpoint remains the submitted prediction with a direct gradient path;
the only added signal is softened T=1 endpoint agreement with the temporal
teacher. No arithmetic process state or intermediate label is supplied.
"""
    ...
    def forward(
        ...
        if self.training:
            with torch.no_grad():
                ...
                if t1_rows.any():
                    teacher_candidate, _ = self.teacher_square(
                        initial_register[t1_rows],
                        modulus[t1_rows],
                        self.training_temperature,
                    )
                    teacher_register[t1_rows] = teacher_candidate
...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    ...
    consistency = _teacher_consistency(batch.logits, teacher_logits, t1_mask)
