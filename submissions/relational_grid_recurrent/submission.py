"""Pairwise relational-grid reasoner for repeated composition.

Every pair of digit positions becomes a learned relation cell containing both
source and context endpoints.  A tied two-dimensional gated processor passes
messages across this grid, and generic output queries attend to all relation
cells.  The architecture supplies pairwise relational capacity but encodes no
multiplication table, significance reduction, carries, comparison, or modular
arithmetic.  The complete learned transition is reused for every outer step.
"""
...
class GridUpdate(nn.Module):
    """One tied message-passing update over a two-dimensional relation grid."""

    def __init__(self) -> None:
        super().__init__()
        self.norm = RMSNorm(HIDDEN)
        self.local = nn.Linear(5 * HIDDEN, 3 * HIDDEN, bias=False)
    ...
    def forward(self, state: Tensor, context: Tensor) -> Tensor:
        ...
        row_zero = torch.zeros_like(normalized[:, :1])
        column_zero = torch.zeros_like(normalized[:, :, :1])
        up = torch.cat((row_zero, normalized[:, :-1]), dim=1)
        down = torch.cat((normalized[:, 1:], row_zero), dim=1)
        left = torch.cat((column_zero, normalized[:, :, :-1]), dim=2)
        right = torch.cat((normalized[:, :, 1:], column_zero), dim=2)
        local = self.local(
            torch.cat((normalized, up, down, left, right), dim=-1)
        )
        global_state = normalized.mean(dim=(1, 2), keepdim=True)
...
class RelationalTransition(nn.Module):
    """Process all source/context digit pairs and query an output register."""
    ...
    def __init__(self, digits: int) -> None:
        ...
        self.pair_mix = nn.Linear(6 * HIDDEN, HIDDEN, bias=False)
        self.pair_norm = RMSNorm(HIDDEN)
        self.seed = nn.Parameter(torch.empty(1, 1, 1, HIDDEN))
        self.query_seed = nn.Parameter(torch.empty(1, 1, HIDDEN))
        self.cell = GridUpdate()
        self.query_norm = RMSNorm(HIDDEN)
        self.key_value_norm = RMSNorm(HIDDEN)
    ...
    def _decode(self, state: Tensor, boundary: Tensor) -> Tensor:
        batch = state.shape[0]
        queries = self.query(
            self.query_norm(self.query_seed + boundary)
        ).view(batch, self.digits, HEADS, self.head_width).transpose(1, 2)
        memory = self.key_value_norm(state.flatten(1, 2))
        keys = self.key(memory).view(
            batch, self.digits * self.digits, HEADS, self.head_width
        ).transpose(1, 2)
        values = self.value(memory).view(
            batch, self.digits * self.digits, HEADS, self.head_width
        ).transpose(1, 2)
        attended = F.scaled_dot_product_attention(queries, keys, values)
        attended = attended.transpose(1, 2).reshape(
            batch, self.digits, HIDDEN
        )
        return self.output(self.output_norm(self.readout(attended)))
    ...
    def forward(
        ...
        boundary = self._boundaries(source).expand(batch, -1, -1)
        row_source = source_features[:, :, None].expand(-1, -1, self.digits, -1)
        column_source = source_features[:, None, :].expand(-1, self.digits, -1, -1)
        row_modulus = modulus_features[:, :, None].expand(-1, -1, self.digits, -1)
        column_modulus = modulus_features[:, None, :].expand(-1, self.digits, -1, -1)
        row_boundary = boundary[:, :, None].expand(-1, -1, self.digits, -1)
        column_boundary = boundary[:, None, :].expand(-1, self.digits, -1, -1)
        context = self.pair_norm(
            self.pair_mix(
                torch.cat(
                    (
                        row_source,
...
class Model(nn.Module):
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.transition = RelationalTransition(self.digits)
