"""Product-aware ripple squaring with confidence-weighted EMA consistency.
...
Soft endpoint agreement is weighted continuously by the teacher's entropy-based
certainty above a uniform digit distribution, without consulting labels. No
...
def _teacher_consistency(student: Tensor, teacher: Tensor, valid: Tensor) -> Tensor:
    ...
    # Confidence is zero for a uniform distribution over decimal digits and
    # approaches one as the teacher concentrates on a digit. It uses no target.
    digit_probability = teacher_probability[
        ..., DIGIT_OFFSET : DIGIT_OFFSET + NUM_DIGITS
    ]
    digit_probability = digit_probability / digit_probability.sum(
        dim=-1, keepdim=True
    ).clamp_min(1e-8)
    entropy = -(
        digit_probability * digit_probability.clamp_min(1e-8).log()
    ).sum(dim=-1)
    token_confidence = (1.0 - entropy / math.log(NUM_DIGITS)).clamp(0.0, 1.0)

    valid_weight = valid.to(losses.dtype)
    valid_count = valid_weight.sum(dim=1).clamp_min(1.0)
    row_confidence = (
        token_confidence * valid_weight
    ).sum(dim=1) / valid_count
    # Geometric mixing prevents one uncertain digit from erasing a confident
    # row while still reducing the entire row when its teacher is diffuse.
    confidence = torch.sqrt(
        token_confidence * row_confidence[:, None]
    )
    weights = valid_weight * confidence
    return (losses * weights).sum() / valid_weight.sum().clamp_min(1.0)
