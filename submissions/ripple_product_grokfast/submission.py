"""Product-aware ripple/EMA squaring with slow-gradient amplification.
...
The optimizer additionally keeps a long-horizon EMA of every student gradient
and adds that slow component to the current gradient after a short burn-in.
This tests whether directions shared across many unrelated prompts encode the
reusable rule while fast-varying directions drive example memorization.  No
...
class GrokfastAdamW:
    """AdamW after amplifying gradient directions persistent across batches."""

    def __init__(
        self,
        inner: torch.optim.AdamW,
        beta: float = GROKFAST_BETA,
        amplification: float = GROKFAST_LAMBDA,
        warmup: int = GROKFAST_WARMUP,
    ) -> None:
        self.inner = inner
        self.optimizers = [inner]
    ...
    def step(self) -> None:
        self.updates += 1
        pending: list[tuple[Tensor, Tensor]] = []
        for group in self.param_groups:
            for parameter in group["params"]:
                gradient = parameter.grad
                if gradient is None:
                    continue
                state = self.inner.state[parameter]
                if "slow_gradient" not in state:
                    pending.append((parameter, gradient.detach().clone()))
                    continue
                slow = state["slow_gradient"]
                slow.mul_(self.beta).add_(gradient, alpha=1.0 - self.beta)
                if self.updates > self.warmup:
                    gradient.add_(slow, alpha=self.amplification)
        self.inner.step()
        for parameter, gradient in pending:
            self.inner.state[parameter]["slow_gradient"] = gradient
