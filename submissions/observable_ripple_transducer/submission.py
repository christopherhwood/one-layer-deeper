"""Endpoint-trained observable digit-belief ripple transducer.

The full learned digit-pair outer product is grouped by significance and its
columns are consumed by alternating tied forward/reverse scans. The only state
that persists between scans is a normalized distribution over the ten actual
digits at every position. A small controller exists only within one scan and
cannot retain a prompt-specific latent code. All transition weights remain
random-initialized and endpoint-trained; the same transition is reused for T.
"""
...
class ProductRippleSquare(nn.Module):
    """One square represented only by repeated observable-belief updates."""
    ...
    def __init__(self, width: int) -> None:
        ...
        self.scan_input = nn.Linear(
            2 * NUM_DIGITS + HIDDEN + 2, HIDDEN, bias=False
        )
        self.scan = nn.GRU(
            HIDDEN,
            HIDDEN,
        ...
        self.digit_delta = nn.Linear(HIDDEN, NUM_DIGITS)
        self.digit_gate = nn.Linear(HIDDEN, NUM_DIGITS)
    ...
    def forward(
        ...
        register = register.clamp_min(0.0)
        register = register / register.sum(dim=-1, keepdim=True).clamp_min(1.0)
        modulus = modulus.clamp_min(0.0)
        modulus = modulus / modulus.sum(dim=-1, keepdim=True).clamp_min(1.0)
        ...
        boundary[0, 0] = 1.0
        boundary[-1, 1] = 1.0
        boundary = boundary[None].expand(register.shape[0], -1, -1)
        state_logits = (0.9 * register + 0.01).log()

        phase_logits: list[Tensor] = []
        for column_index in range(self.columns - 1, -1, -1):
            column = product[:, column_index, None, :].expand(
                -1, self.width, -1
            )
            for micro_step in range(MICRO_STEPS):
                state = F.softmax(state_logits / temperature, dim=-1)
                scan_input = self.scan_input(
                    torch.cat((state, modulus, column, boundary), dim=-1)
                )
                if micro_step % 2:
                    scan_input = scan_input.flip(1)
                scanned, _ = self.scan(scan_input)
                if micro_step % 2:
                    scanned = scanned.flip(1)
                scanned = self.scan_norm(scanned)
                rate = torch.sigmoid(
                    self.digit_gate(scanned) + self.gate_bias
                )
                state_logits = state_logits + rate * self.digit_delta(scanned)
                state_logits = F.log_softmax(state_logits, dim=-1)
                if column_index == 0:
                    phase_logits.append(state_logits)
        probabilities = F.softmax(state_logits / temperature, dim=-1)
