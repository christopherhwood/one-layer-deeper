"""Endpoint-trained product-column serial reducer.
...
tape.  A unidirectional recurrent carrier and terminal-state broadcast provide
length-independent local propagation and global control.  All arithmetic state
and updates remain learned from endpoint labels; the architecture supplies only
the generic streaming order, residual memory, and weight sharing.  The same
learned transition is reused for every T.
"""
    ...
    def forward(
        ...
        for column_index in range(self.columns - 1, -1, -1):
            ...
            for _ in range(MICRO_STEPS):
                ...
                control = self.controller(terminal[-1])[:, None, :]
