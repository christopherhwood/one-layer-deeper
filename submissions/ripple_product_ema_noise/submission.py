"""Product-aware ripple squaring with a clean EMA teacher.
...
complete modular-square transition. During training, modest dropout perturbs
only the student's internal projected features, while a frozen EMA copy runs
clean on the same untouched digit registers and complete requested T path. The
...
class ProductRippleSquare(nn.Module):
    ...
    def forward(
        ...
        state = F.dropout(
            state, p=feature_dropout, training=feature_dropout > 0.0
        )
...
class Model(nn.Module):
    ...
    def _run_square(
        ...
        for outer_step in range(loops):
            ...
            candidate, phases = square(
                transition_input, modulus, temperature, feature_dropout
            )
            if outer_step == 0:
                first_phases = phases
    ...
    def forward(
        ...
        register, first_square_phases = self._run_square(
            ...
            temperature,
            self.training,
            STUDENT_FEATURE_DROPOUT if self.training else 0.0,
        )
