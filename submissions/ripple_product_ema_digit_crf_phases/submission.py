"""Product-aware ripple/EMA squaring with shared endpoint/phase digit CRFs.
...
training adds partial-suffix likelihood at the endpoint and legitimate T=1
refinement phases, while evaluation uses final-state Viterbi decoding.
...
class Model(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            auxiliary["phase_crfs"] = tuple(
                self.square.crf_energies(state, unary)
                for state, unary in zip(first_square_states, first_square_phases)
            )
            with torch.no_grad():
                teacher_register, _, _, _, _ = self._run_square(
                    self.teacher_square,
                    initial_register,
                    modulus,
                    t_values,
...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    ...
    t1_rows = t_values == 1
    phase_terms = []
    for full_logits, phase_crf in zip(
        batch.auxiliary["phase_logits"], batch.auxiliary["phase_crfs"]
    ):
        phase_ce = _masked_loss(
            _target_aligned(full_logits, batch), batch.labels, t1_mask
        )
        phase_structured = _partial_suffix_crf_nll(
            batch, phase_crf, t1_rows
        )
        phase_terms.append(phase_ce + CRF_WEIGHT * phase_structured)
    if phase_terms:
