"""Product-aware ripple/EMA squaring with cross-example gradient consensus.
...
Two evaluator-owned passes use disjoint row halves of the same untouched
minibatch.  The optimizer attenuates parameter-gradient coordinates whose signs
disagree between halves, targeting shared-function directions rather than
prompt-specific memorization. No arithmetic process state or intermediate
label is supplied.
"""
...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    ...
    row_mask = row_index.remainder(2) == int(batch.auxiliary["loss_half"])
...
class ConsensusAdamW(torch.optim.AdamW):
    """Attenuate coordinates that disagree across independent row halves."""
    ...
    def between_backward_passes(self, context: BackwardPassContext) -> None:
        ...
        if self.model.loss_half != 0:
            raise RuntimeError("first consensus pass must use row half zero")
    ...
    def step(self, closure=None):
        ...
        if len(self.first_gradients) != len(parameters):
            ...
            raise RuntimeError("first-half gradients were not captured")
        for parameter, first in zip(parameters, self.first_gradients):
            second = parameter.grad
            if first is None:
                continue
            if second is None:
                parameter.grad = first.mul(0.1)
                continue
            mean = first.add(second).mul_(0.5)
            agreement = first.mul(second).gt(0).to(mean.dtype)
            # Retain a small residual so sparse/rare-but-real transitions are
            # not permanently starved; agreement receives the full update.
            mean.mul_(agreement.mul_(0.9).add_(0.1))
            parameter.grad = mean
        try:
