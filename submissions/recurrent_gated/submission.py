"""Weight-tied Transformer with step conditioning and gated refinement."""
...
class Model(nn.Module):
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.step_embedding = nn.Embedding(NUM_LOOPS, D_MODEL)
        self.block = Block()
    ...
    def forward(
        ...
        for step in range(NUM_LOOPS):
            conditioned = x + self.step_embedding.weight[step]
            proposal = self.block(conditioned, attention_mask)
            update_gate = torch.sigmoid(self.gate(self.gate_norm(conditioned)))
            x = x + update_gate * (proposal - conditioned)
