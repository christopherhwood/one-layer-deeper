"""Generic recurrent transition with endpoint-anchored self-distillation.
...
Earlier refinements also match the detached final refinement distribution. This
is a generic fixed-point consistency signal: it adds no labels beyond the real
endpoint. The same complete transition is reused exactly T times; T is only a
compute-control signal and is never exposed to the learned transition.
"""
...
def _masked_consistency(
    student: Tensor,
    teacher: Tensor,
    valid: Tensor,
) -> Tensor:
    """KL from a detached later phase to an earlier phase."""

    temperature = CONSISTENCY_TEMPERATURE
    teacher_probabilities = F.softmax(teacher.detach() / temperature, dim=-1)
    student_log_probabilities = F.log_softmax(student / temperature, dim=-1)
    token_kl = F.kl_div(
    ...
    return (temperature * temperature) * (token_kl * weights).sum() / (
...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    ...
    aligned_phases = [
        _target_aligned(phase, batch) for phase in phases
    ]
    deep_terms = [
    ...
    teacher = aligned_phases[-1]
    consistency_terms = [
        _masked_consistency(phase, teacher, batch.valid_mask)
        for phase in aligned_phases[:-1]
    ]
    deep_endpoint = torch.stack(deep_terms).mean()
    consistency = torch.stack(consistency_terms).mean()
    return endpoint + 0.5 * deep_endpoint + CONSISTENCY_WEIGHT * consistency
