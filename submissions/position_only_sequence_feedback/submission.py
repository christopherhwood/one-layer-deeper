"""Position-only recurrent mixer with fixed whole-sequence feedback.
...
refinement depth, and outer T. Every refinement state is also aligned with a
fixed random code for the complete real endpoint sequence, using digit identity
and position from the least-significant end.
"""
...
class ProductPositionSquare(nn.Module):
    def __init__(self, width: int) -> None:
        ...
        sequence_codebook = torch.empty(
            self.width, NUM_DIGITS, HIDDEN
        )
        nn.init.normal_(sequence_codebook)
        sequence_codebook = F.normalize(sequence_codebook, dim=-1)
        self.register_buffer("sequence_codebook", sequence_codebook)
    ...
    def forward(
        ...
        for _ in range(REFINEMENT_STEPS):
            ...
            phase_states.append(state[:, : self.width])
        return (
...
class Model(nn.Module):
    ...
    def forward(
        ...
        first_phase_states: tuple[Tensor, ...] = ()
        for outer_step in range(loops):
            ...
            candidate, phases, phase_states = self.square(
                transition_input, modulus, temperature
            )
            if outer_step == 0:
                first_phases = phases
                first_phase_states = phase_states
        ...
        return self._place(register.clamp_min(1e-8).log(), lengths, prompt), {
            ...
            "phase_states": first_phase_states,
            "sequence_codebook": self.square.sequence_codebook,
        }
...
def _sequence_code_loss(
    state: Tensor,
    labels: Tensor,
    valid: Tensor,
    codebook: Tensor,
) -> Tensor:
    counts = valid.sum(dim=1).clamp(max=state.shape[1])
    rank_from_right = (
        valid.long().flip(1).cumsum(dim=1).flip(1) - 1
    ).clamp(0, codebook.shape[0] - 1)
    digits = (labels - DIGIT_OFFSET).clamp(0, NUM_DIGITS - 1)
    target_parts = codebook[rank_from_right, digits]
    target = (
        target_parts * valid.unsqueeze(-1)
    ).sum(dim=1) / counts.clamp_min(1).sqrt().unsqueeze(-1)

    state_positions = torch.arange(
        state.shape[1], device=state.device
    )[None]
    state_valid = state_positions < counts[:, None]
    pooled = (
        state * state_valid.unsqueeze(-1)
    ).sum(dim=1) / counts.clamp_min(1).sqrt().unsqueeze(-1)
    ...
    row_weight = (counts > 0).to(distance.dtype)
    return (distance * row_weight).sum() / row_weight.sum().clamp_min(1.0)
...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    ...
    sequence_terms = [
        _sequence_code_loss(
            ...
            batch.labels,
            t1,
            batch.auxiliary["sequence_codebook"],
        )
        for state in batch.auxiliary["phase_states"]
    ]
    if sequence_terms:
        loss = (
            loss
            + SEQUENCE_FEEDBACK_WEIGHT
