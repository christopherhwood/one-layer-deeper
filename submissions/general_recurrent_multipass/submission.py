"""General recurrent endpoint learner with evaluator-owned two-pass gradients.

The first evaluator-owned pass trains short, detached refinement segments.  The
second trains the same transition end to end.  A custom AdamW subclass averages
the two gradients immediately before its ordinary optimizer update.  No pass is
started here: both forwards, losses, and backwards remain evaluator-owned.
"""
...
class MultipassAdamW(torch.optim.AdamW):
    """Average detached-segment and full-segment evaluator-owned gradients."""

    def __init__(self, groups, model: Model, **kwargs) -> None:
        super().__init__(groups, **kwargs)
        self.model = model
        self.saved_gradients: list[Tensor | None] = []

    @torch.no_grad()
    def between_backward_passes(self, context: BackwardPassContext) -> None:
        if context.pass_index != 1 or context.total_passes != 2:
            raise RuntimeError("multipass optimizer requires exactly two passes")
        self.saved_gradients = [
            None if parameter.grad is None else parameter.grad.detach().clone()
            for group in self.param_groups
            for parameter in group["params"]
        ]
        self.model.detach_segments = False
    ...
    def step(self, closure=None):
        ...
        if len(self.saved_gradients) != len(parameters):
            raise RuntimeError("first-pass gradients were not captured")
        for parameter, saved in zip(parameters, self.saved_gradients):
            if saved is None:
                continue
            if parameter.grad is None:
                parameter.grad = saved
            else:
                parameter.grad.add_(saved).mul_(0.5)
        result = super().step(closure)
        self.saved_gradients = []
        self.model.detach_segments = True
        return result
