"""Position-routed, content-gated recurrent modular-square learner.

Communication weights depend only on relative position and are shared across
examples, refinement depth, and outer T. Once a fixed-routed context arrives at
each position, a multiplicative learned gate may condition the local update on
digit and modulus content. This separates reusable addressing from conditional
computation without supplying arithmetic process labels.
"""
...
class PositionOnlyRouter(nn.Module):
    """Global learned relative routing with content-independent weights."""
    ...
    def forward(self, state: Tensor) -> Tensor:
        ...
        context = torch.einsum("hij,bhjd->bhid", routing, values)
        return self.output(context.transpose(1, 2).reshape_as(state))


class ContentGatedUpdate(nn.Module):
    """Value-dependent computation applied only after fixed routing."""

    def __init__(self) -> None:
        super().__init__()
        self.state_norm = RMSNorm(HIDDEN)
        self.context_norm = RMSNorm(HIDDEN)
        self.gate = nn.Linear(3 * HIDDEN, HIDDEN)
        self.candidate = nn.Linear(3 * HIDDEN, HIDDEN)
        self.output = nn.Linear(HIDDEN, HIDDEN, bias=False)
        self.step_scale = nn.Parameter(torch.empty(HIDDEN))
        nn.init.normal_(self.step_scale, mean=-2.0, std=0.02)

    def forward(self, state: Tensor, context: Tensor) -> Tensor:
        local = self.state_norm(state)
        routed = self.context_norm(context)
        features = torch.cat((local, routed, local * routed), dim=-1)
        gate = torch.sigmoid(self.gate(features))
        candidate = torch.tanh(self.candidate(features))
        return state + torch.sigmoid(self.step_scale) * self.output(
            gate * candidate
        )


class ProductPositionGatedSquare(nn.Module):
    def __init__(self, width: int) -> None:
        super().__init__()
        self.width = width
        ...
        self.route_norm = RMSNorm(HIDDEN)
        self.router = PositionOnlyRouter(self.columns)
        self.update = ContentGatedUpdate()
    ...
    def forward(
        ...
        for _ in range(REFINEMENT_STEPS):
            context = self.router(self.route_norm(state))
            state = self.update(state, context)
...
class Model(nn.Module):
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.square = ProductPositionGatedSquare(self.width)
