"""Product-aware ripple squaring with a temporal EMA probability ensemble.
...
complete modular-square transition. During training, a frozen EMA copy runs on
the same untouched digit registers in the same forward pass. Its already
computed endpoint is mixed with the student endpoint as an ordinary probability
ensemble, ramping to 25% teacher weight with the existing consistency schedule.
Evaluation independently runs both trajectories and uses the fixed 25% teacher
weight. The student always has a nonzero direct endpoint gradient, while the
existing consistency KL still compares the unmixed endpoints. No arithmetic
...
class Model(nn.Module):
    ...
    def forward(
        ...
        student_logits = self._place_logits(
        ...
        teacher_weight = (
            TEACHER_MIX * self.consistency_scale
            if self.training
            else TEACHER_MIX
        )
        mixed_register = torch.lerp(register, teacher_register, teacher_weight)
        logits = self._place_logits(
            mixed_register.clamp_min(1e-8).log(), lengths, prompt_length
        )
        if self.training:
            auxiliary["student_logits"] = student_logits
...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    ...
    student_logits = _target_aligned(batch.auxiliary["student_logits"], batch)
    consistency = _teacher_consistency(student_logits, teacher_logits, valid)
